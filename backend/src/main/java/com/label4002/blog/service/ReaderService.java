...
public class ReaderService {
    ...
    public void recordReadHistory(Long userId, Long postId) {
        if (!readHistoryRepository.existsByUserIdAndPostId(userId, postId)) {
    ...
    @Transactional(readOnly = true)
    public Page<ReadHistoryEntity> getReadHistory(Long userId, int page, int size) {
        return readHistoryRepository.findByUserIdOrderByReadAtDesc(userId, PageRequest.of(page, size));
    }

    @Transactional(readOnly = true)
    public Page<FavoriteEntity> getFavorites(Long userId, int page, int size) {

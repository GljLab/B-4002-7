...
import type { ReaderPublicProfileDTO } from '../types'
...
async function loadTabData() {
  ...
  try {
    if (activeTab.value === 'footprint' && profile.value.readCount > 0) {
      const res = await getReadHistory(currentPage.value, pageSize)
      historyItems.value = res.items
      pageTotal.value = res.total
    } else if (activeTab.value === 'favorites') {
      const res = await getFavorites(currentPage.value, pageSize)
          ...
          <el-tab-pane label="浏览足迹" name="footprint">
            ...
            <div v-else class="item-list">
              <router-link
                ...
                class="item-row"
              >
                <span class="item-title">文章 #{{ item.postId }}</span>
